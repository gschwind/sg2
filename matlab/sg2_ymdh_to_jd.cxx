...
void mexFunction( int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[] )
{
	...
	double * year = reinterpret_cast<double *>(mxGetData(prhs[0]));
	double * month = reinterpret_cast<double *>(mxGetData(prhs[1]));
	double * day = reinterpret_cast<double *>(mxGetData(prhs[2]));
	double * hours = reinterpret_cast<double *>(mxGetData(prhs[3]));

	plhs[0] = mxCreateDoubleMatrix(mxGetM(prhs[0]), mxGetN(prhs[0]), mxREAL);
	double * jd = reinterpret_cast<double *>(mxGetData(plhs[0]));

	int const n = mxGetM(prhs[0]) * mxGetN(prhs[0]);
	for (int i = 0; i < n; ++i) {
		jd[i] = sg2::julian{sg2::ymdh(year[i], month[i], day[i], hours[i])}.value;
	}
