...
void SG2_heliocentric_coordinates_set_hcoord(S_SG2_DATE_JD *p_jd,
	...
	for (k = 0; k < p_jd->n; k++) {
		...
		x = (p_jd->jd_tt[k] - SG2_precomputed_heliocentric_coordinates_j0)
				/ SG2_precomputed_heliocentric_coordinates_dj;
		x0 = floor(x);
		dx = x - x0;

		idx0 = (short) x0;
		if ((idx0 < 0) || (idx0 > SG2_precomputed_heliocentric_coordinates_nj
				- 1)) {
		...
		p_hcoord->R[k] = (1.0 - dx)
				* SG2_precomputed_heliocentric_coordinates_R[idx0] + dx
				* SG2_precomputed_heliocentric_coordinates_R[idx0 + 1];
		p_hcoord->L[k] = (1.0 - dx)
